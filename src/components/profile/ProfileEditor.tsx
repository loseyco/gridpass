...
export default function ProfileEditor({ targetUserId }: ProfileEditorProps) {
    ...
    const handleCropComplete = async (croppedBlob: Blob) => {
        if (!cropConfig || !cropImageSrc || !resolvedUserId) return;
        ...
        try {
            ...
            // Use resolvedUserId for the path to ensure admins upload to correct folder
            const filePath = isAvatar ? `${resolvedUserId}/${fileName}` : `${resolvedUserId}/cover/${fileName}`;
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold">
                        {targetUserId ? `Edit User Profile` : `Edit Profile`}
                    </h1>
                    <p className="text-neutral-400">
                        {targetUserId ? `Editing user @${formData.username || 'unknown'}` : `Manage your identity across the GridPass ecosystem.`}
                    </p>
                </div>
                ...
                <div className="lg:col-span-3 space-y-6">
                    {activeSection === 'career' ? (
                        ...
                        <div className="bg-neutral-900 border border-white/5 rounded-xl p-6 md:p-8 animate-fade-in">
                            ...
                            {resolvedUserId && <MediaGalleryEditor userId={resolvedUserId} />}
                        </div>
                    ) : (
                        SCHEMA_CATEGORIES.map(cat => (
