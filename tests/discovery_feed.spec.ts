...
test.describe('Gridpass Phase 5: Discovery Feed & Leaderboard E2E Suite', () => {
  ...
  test('Ecosystem leaderboards navigation and ranking logs', async ({ page }, testInfo) => {
    ...
    // Verify main header
    await expect(page.locator('h1')).toContainText(/(Driver Leaderboard|Ecosystem Leaderboards)/i);
    await expect(page.locator('button:has-text("Grid Credits")')).toBeVisible();

    // Verify Rank 1 driver is present
    await expect(page.locator('text=pjlosey').or(page.locator('text=Marcus')).first()).toBeVisible();

    // Click Spotters tab
    await page.click('button:has-text("Spotters")');
